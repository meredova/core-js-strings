...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let b = 0; b < vovels.length; b += 1) {
      if (str[i] === vovels[b]) {
        count += 1;
      }
    }
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentenceArr.length; i += 1) {
    const currentWord = sentenceArr[i].replace(/[^\w]/g, '');
...
function extractEmails(str) {
  const emailsArray = str.split(';').map((email) => email.trim());
  const validEmails = emailsArray.filter((email) => email !== '');
  return validEmails;
}
